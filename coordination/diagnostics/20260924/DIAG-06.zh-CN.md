...
依赖：DIAG-02, DIAG-03, DIAG-05
基线规则：DIAG-05 审核通过的精确 HEAD；消费已审核 DIAG-02/03 的脱敏样例和契约摘要。
...
新增独立 Go 命令读取多个 JSONL/@diag 导出，不访问网络/生产。支持 trace 和带来源作用域 callerRequestId 查询，文本树及 JSON 报告。展示资源身份、请求/call/attempt 分开计数、verified/unverified/missing/ambiguous/context mismatch 等证据分类和 DEBUG 覆盖。traceId 只筛选候选，不直接合并成可信树；事件用规范键去重，内容冲突保留。旧文本日志只能作为明确降级的证据，不能臆造身份。

## 验证与交付

跨实例/重启/重复导入、坏行与混流、日志截断、时钟偏差、复用/伪造 ID、多个调用方、缺父节点/双边冲突、未知版本、字段缺失/零、业务尝试与网络调用数；Go 测试、完整回归与 server/工具编译。
...
逐边应用 graph 向量，不把离线 oracle 的单边投影直接当完整工具；保留所有冲突 payload 与来源，缺本地拥有者/循环/来源未知单独显示。坏行仅报告来源位置/长度/原因，原文留本地；禁止导出原始坏行或旧文本日志。未知版本不强行降级为 v1。


R1 完整性补充：accessCapture=unknown/none 及矛盾终局在无已知丢失时为 unknown；终局截断存根算“存在但不完整”，terminalMissing=false、debugCoverage=partial，不能证明成功或 verified。

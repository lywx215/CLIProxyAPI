...
公共 schema 是投影，不覆盖既有 schemaVersion/seq/deliveryOutcome。必须区分 sinkDroppedTotal 和单 span 丢弃；旧 wall-clock/browser 指标留在原日志/统计，公共 timing 只收新可靠单调时钟指标，未知 null，不混标来源。基础终局不放入 DEBUG 清空队列，禁止把公共 holder 塞入 proxyRequest 或添加浏览器 span。

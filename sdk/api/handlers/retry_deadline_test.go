	...
	for _, quota := range []bool{false, true} {
		...
		for _, streaming := range []bool{false, true} {
			...
			if !strings.Contains(err.Error(), "gpt-5.6-luna") {
				t.Fatal("internal model context was lost")
			}
			if !strings.Contains(recorder.Body.String(), FixedErrorMessage(wantStatus)) {
				t.Fatal("client response lost the safe status message")
			}
		}
	}
}

...
func TestExecutionErrorMessageSanitizesMarkedUpstreamResponse(t *testing.T) {
	tests := []struct {
		name        string
		status      int
		body        []byte
		...
		{name: "text", status: http.StatusBadGateway, body: []byte("https://private.example token=secret"), contentType: "application/json"},
		{name: "multiline", status: http.StatusTooManyRequests, body: []byte("first line\r\nsecond line\n"), contentType: "application/json"},
		{name: "credits", status: http.StatusPaymentRequired, body: []byte(`{"error":{"message":"secret upstream account"}}`), contentType: "application/json"},
		{name: "server", status: http.StatusInternalServerError, body: []byte("private credential path"), contentType: "application/json"},
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			...
			if msg == nil || msg.DirectResponse || msg.Error != errUpstream {
				t.Fatalf("executionErrorMessage() = %#v, want sanitized response retaining original error", msg)
			}
			NewBaseAPIHandlers(nil, nil).WriteErrorResponse(c, msg)
			if recorder.Code != tt.status {
				t.Fatalf("status = %d, want %d", recorder.Code, tt.status)
			}
			wantBody := BuildErrorResponseBody(tt.status, string(tt.body))
			if got := recorder.Body.Bytes(); !bytes.Equal(got, wantBody) {
				t.Fatalf("body = %q, want sanitized body %q", got, wantBody)
			}
...
func TestBuildErrorResponseBodyWithError_TerminalAuthEnforcesContractOnJSONInput(t *testing.T) {
	...
	if payload.Error.Message != FixedErrorMessage(http.StatusServiceUnavailable) {
		t.Fatalf("message = %q, want fixed safe message", payload.Error.Message)
	}

	// Non-terminal errors also keep diagnostics out of the client response.
	normalBody := BuildErrorResponseBodyWithError(http.StatusInternalServerError, jsonErrText, errors.New("normal error"))
	if strings.Contains(string(normalBody), "token refresh failed") || !strings.Contains(string(normalBody), FixedErrorMessage(http.StatusInternalServerError)) {
		t.Fatalf("expected safe non-terminal error, got %s", string(normalBody))
	}
}

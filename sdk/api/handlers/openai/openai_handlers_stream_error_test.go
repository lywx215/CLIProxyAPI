...
func runOpenAIStreamErrorTest(t *testing.T, endpoint string, body string) {
	...
	for i := 0; i < 100; i++ {
		...
		go func(idx int) {
			...
			if strings.Contains(recorder.Body.String(), "upstream failed before first payload") || !strings.Contains(recorder.Body.String(), "Internal server error") {
				t.Errorf("[%s] request %d did not return the fixed safe error: status=%d body=%q", endpoint, idx, recorder.Code, recorder.Body.String())
			}
		}(i)
	}
	wg.Wait()

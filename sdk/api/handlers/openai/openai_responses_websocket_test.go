...
func TestResponsesWebsocketSendsJSONErrorOnUpstreamCyberPolicyDisconnect(t *testing.T) {
	...
	if message := gjson.GetBytes(payload, "error.message").String(); message != handlers.FixedErrorMessage(http.StatusBadRequest) || strings.Contains(message, "cybersecurity risk") {
		t.Fatalf("error.message = %q, want fixed safe message", message)
	}
...
func TestResponsesWebsocketExposesTerminalOAuthError(t *testing.T) {
	...
	if gjson.GetBytes(payload, "error.message").String() != handlers.FixedErrorMessage(http.StatusServiceUnavailable) {
		t.Fatalf("unsafe terminal OAuth message: %s", payload)
	}
}
...
func TestResponsesWebsocketExposesItemNotPersistedAndRecoversOnReconnect(t *testing.T) {
	...
	if msg := gjson.GetBytes(errorResponse, "error.message").String(); msg != handlers.FixedErrorMessage(http.StatusNotFound) {
		t.Fatalf("error.message = %q, want fixed safe message", msg)
	}

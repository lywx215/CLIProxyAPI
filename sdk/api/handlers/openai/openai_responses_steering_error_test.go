...
func TestResponsesSteeringErrorRecoveryIntegration(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			rejection := []byte(`{"type":"error","status":400,"event_id":"rejected-create","sequence_number":42,"error":{"type":"invalid_request_error","code":"invalid_request","retryable":true,"message":"secret diagnostic https://internal.invalid","debug":"private"}}`)
			...
			for {
				...
				case "error":
					errorsSeen++
					if gjson.GetBytes(p, "error.message").String() != handlers.FixedErrorMessage(http.StatusBadRequest) || strings.Contains(string(p), "private") || strings.Contains(string(p), "internal.invalid") {
						t.Errorf("unsafe error payload: %s", p)
					}
					if tc.enabled && !tc.initial {
						if gjson.GetBytes(p, "event_id").String() != "rejected-create" || gjson.GetBytes(p, "sequence_number").Int() != 42 || !gjson.GetBytes(p, "error.retryable").Bool() || gjson.GetBytes(p, "error.code").String() != "invalid_request" {
							t.Errorf("recoverable error metadata lost: %s", p)
						}
					}
					if recoverable {
						send("CORRECTED")

...
func TestResponsesHandlerEmitsFailureWhenExecutorStopsAfterPartialOutput(t *testing.T) {
	...
	if !strings.Contains(body, handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(body, "unexpected EOF") {
		t.Fatalf("handler terminal failure did not use the fixed safe message: %q", body)
	}
}
...
func TestResponsesHandlerSanitizesErrorBeforeFirstFrame(t *testing.T) {
	...
	body := recorder.Body.String()
	if recorder.Code == http.StatusOK || !strings.Contains(body, "upstream_failed") || !strings.Contains(body, handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(body, "initial upstream failure") {
...
func TestResponsesHandlerDoesNotCommitHeadersForIncompleteFirstFrame(t *testing.T) {
	...
	if !strings.Contains(recorder.Body.String(), handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(recorder.Body.String(), "upstream failed before first complete frame") {
...
func TestResponsesHandlerRejectsStreamClosedBeforeFirstPayload(t *testing.T) {
	...
	if !strings.Contains(recorder.Body.String(), handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(recorder.Body.String(), "closed before first payload") {
...
func TestResponsesHandlerDoesNotLoseErrorBeforeFirstPayload(t *testing.T) {
	...
	for i := 0; i < 100; i++ {
		...
		if !strings.Contains(recorder.Body.String(), handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(recorder.Body.String(), "upstream failed before first payload") {
...
func TestForwardResponsesStreamExposesTransportErrorAfterOutputForCodex(t *testing.T) {
	...
	h.forwardResponsesStream(c, flusher, func(error) {}, data, errs, framer, nil)
	body := recorder.Body.String()
	if !strings.Contains(body, "event: response.failed") {
		t.Fatalf("transport failure ended without response.failed: %q", body)
	}
	if strings.Contains(body, "unexpected EOF") || !strings.Contains(body, "Upstream service error") {
		t.Fatalf("response.failed did not use the fixed safe message: %q", body)
	}
...
func TestForwardResponsesStreamSanitizesDiagnosticErrorDetails(t *testing.T) {
	...
	h.forwardResponsesStream(c, flusher, func(error) {}, data, errs, framer, nil)
	body := recorder.Body.String()
	if !strings.Contains(body, "Upstream service error") || !strings.Contains(body, "upstream_failed") {
		t.Fatalf("client error lost the fixed message or safe structured code: %q", body)
	}
...
func TestForwardResponsesStreamPreservesNestedResponseError(t *testing.T) {
	...
	h.forwardResponsesStream(c, flusher, func(error) {}, data, errs, framer, nil)
	body := recorder.Body.String()
	for _, want := range []string{"Upstream service error", "upstream_failed", "server_error"} {
...
func TestForwardResponsesStreamSanitizesPayloadErrorsAndStopsAtFailure(t *testing.T) {
	for _, tc := range []struct {
		...
		t.Run(tc.name, func(t *testing.T) {
			...
			h.forwardResponsesStream(c, flusher, func(err error) { canceled = err }, data, errs, &responsesSSEFramer{}, nil)
			body := recorder.Body.String()
			if canceled == nil {
			...
			if strings.Count(body, "event: response.failed") != 1 || !strings.Contains(body, "Upstream service error") {
...
func TestForwardResponsesStreamFailsWhenUpstreamClosesWithoutTerminalEvent(t *testing.T) {
	...
	if strings.Contains(body, "closed before a terminal event") || !strings.Contains(body, "Upstream service error") {
		t.Fatalf("response.failed did not use the fixed safe message: %q", body)
	}
}

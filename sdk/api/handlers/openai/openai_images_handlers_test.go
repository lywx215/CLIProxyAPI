...
func TestWriteImagesStreamErrorEventSanitizesPayload(t *testing.T) {
	...
	if strings.Contains(body, "image-secret") || len(body) > 4096 || !strings.Contains(body, handlers.FixedErrorMessage(http.StatusBadGateway)) || !strings.Contains(body, "upstream_failed") {

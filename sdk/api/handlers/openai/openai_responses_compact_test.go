...
func TestOpenAIResponsesCompactTransientFailureDoesNotCooldownAuthAndPreservesError(t *testing.T) {
	...
	// 1. Should preserve the upstream status while returning the fixed safe message.
	if resp.Code != http.StatusInternalServerError {
		t.Fatalf("compact status = %d, want %d; body = %s", resp.Code, http.StatusInternalServerError, resp.Body.String())
	}
	if !strings.Contains(resp.Body.String(), handlers.FixedErrorMessage(http.StatusInternalServerError)) || strings.Contains(resp.Body.String(), "compact upstream temporary error") {
		t.Fatalf("compact body = %s, want fixed safe error message", resp.Body.String())
	}
...
func TestOpenAIResponsesCompactRequestFaultStopsFallbackAndPreservesError(t *testing.T) {
	...
	if !strings.Contains(resp.Body.String(), handlers.FixedErrorMessage(http.StatusNotFound)) || strings.Contains(resp.Body.String(), "404 page not found") {
		t.Fatalf("compact body = %s, want fixed safe error message", resp.Body.String())
	}

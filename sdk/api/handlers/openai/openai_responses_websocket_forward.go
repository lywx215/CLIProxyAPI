...
func buildResponsesWebsocketErrorPayload(errMsg *interfaces.ErrorMessage) ([]byte, error) {
	...
	if errMsg != nil && errMsg.Error != nil {
		if original := strings.TrimSpace(errMsg.Error.Error()); original != "" && original != errText {
			log.Debugf("[error-sanitize/websocket] status=%d, fixed=%q, original=%s", status, errText, original)

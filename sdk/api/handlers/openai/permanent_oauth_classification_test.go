	...
	for _, stream := range []bool{false, true} {
		t.Run(fmt.Sprintf("stream=%t", stream), func(t *testing.T) {
			...
			for attempt := 1; attempt <= 3; attempt++ {
				...
				if payload.Error.Message != handlers.FixedErrorMessage(http.StatusServiceUnavailable) {
					t.Errorf("attempt %d: unsafe terminal OAuth message: %s", attempt, payload.Error.Message)
				}
			}

...
func TestResponsesSteeringLocalValidationRecovery(t *testing.T) {
	...
	for !completed {
		...
		case "error":
			...
			switch errorsSeen {
			case 1:
				if gjson.GetBytes(p, "error.message").String() != handlers.FixedErrorMessage(http.StatusBadRequest) {
					t.Errorf("unsafe JSON validation message: %s", p)
				}
				send(`{"type":"unsupported.request"}`)
			case 2:
				if gjson.GetBytes(p, "error.message").String() != handlers.FixedErrorMessage(http.StatusBadRequest) {
					t.Errorf("unsafe type validation message: %s", p)
				}

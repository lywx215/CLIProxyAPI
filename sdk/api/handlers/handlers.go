...
// internalLeakPatterns detects internal infrastructure details that must not be exposed.
...
// SanitizeErrorMessage strips internal network details from server-side errors.
func SanitizeErrorMessage(status int, msg string) string {
	if status >= http.StatusInternalServerError && internalLeakPatterns.MatchString(msg) {
		return "upstream service unavailable"
	}
	return msg
}

// SanitizeJSONErrorPayload redacts internal details in a JSON error envelope.
func SanitizeJSONErrorPayload(status int, raw []byte) []byte {
	...
	if err := json.Unmarshal(raw, &envelope); err != nil || envelope.Error == nil || !internalLeakPatterns.MatchString(envelope.Error.Message) {
		return raw
	}
	...
	if errorObject, ok := payload["error"].(map[string]any); ok {
		errorObject["message"] = "upstream service unavailable"
	}
...
// FixedErrorMessage returns a stable client-safe message for an HTTP status.
...
func summarizeForDebugLog(value string, maxLen int) string {
	if maxLen <= 0 {
		maxLen = 512
	}
	if len(value) <= maxLen {
...
func BuildErrorResponseBodyWithError(status int, errText string, err error) []byte {
	...
	if original := strings.TrimSpace(errText); original != "" && original != fixedMessage && original != http.StatusText(status) {
		log.Debugf("[error-sanitize] status=%d, fixed=%q, original=%s", status, fixedMessage, summarizeForDebugLog(original, 512))
	}
	...
	if json.Unmarshal([]byte(strings.TrimSpace(errText)), &source) == nil {
		if nested, ok := source["error"].(map[string]any); ok {
			source = nested
		} else if response, ok := source["response"].(map[string]any); ok {
			if nested, ok := response["error"].(map[string]any); ok {
				source = nested
			}
		}
		if value, ok := source["type"].(string); ok && safeErrorIdentifier(value) {
			errType = value
		}
		if value, ok := source["code"].(string); ok && safeErrorIdentifier(value) {
			code = value
		}
...
func safeErrorIdentifier(value string) bool {
	value = strings.TrimSpace(value)
	return value != "" && len(value) <= 256 && !internalLeakPatterns.MatchString(value)
}

func safeErrorParam(value any) any {
	param, ok := value.(string)
	if !ok || !safeErrorIdentifier(param) {
		return nil
	}
	return param
}
...
func (h *BaseAPIHandler) GetContextWithCancel(handler interfaces.APIHandler, c *gin.Context, ctx context.Context) (context.Context, APIHandlerCancelFunc) {
	...
	parentCtx = diagnostics.CarryContext(parentCtx, requestCtx)

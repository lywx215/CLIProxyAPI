...
func (m *Manager) MarkResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		// Runtime cooldown and accounting updates must not rewrite credentials on every request.
		// Refresh and explicit auth update flows persist credential changes separately.
		auth.Generation++
		auth.UpdatedAt = now
...
func (m *Manager) recordAvailabilityNeutralResult(ctx context.Context, result Result) {
	...
	if auth, ok := m.auths[result.AuthID]; ok && auth != nil {
		...
		auth.Generation++
		auth.UpdatedAt = now
		// Availability-neutral accounting must not save credentials on every request.
		authSnapshot = auth.Clone()
	}
	m.mu.Unlock()

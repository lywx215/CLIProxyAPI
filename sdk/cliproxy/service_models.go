...
func buildConfigModels[T modelEntry](models []T, ownedBy, modelType, metadataChannel string) []*ModelInfo {
	...
	for i := range models {
		model := models[i]
		name := strings.TrimSpace(model.GetName())
		// Explicit display-name wins; otherwise expose the client-visible alias.
		info := buildConfiguredModelInfo(model, ownedBy, modelType, now, strings.TrimSpace(model.GetAlias()), true)
		if info == nil {
			continue
		}
		alias := info.ID

...
type QuotaExceeded struct {
	...
	// AntigravityCreditsForce requires Claude requests to consume Google One AI credits.
...
type APIKeyRateLimit struct {
	...
	// DefaultRPM is the requests-per-minute limit for keys without an override.
	DefaultRPM int `yaml:"default-rpm" json:"default-rpm"`

	// Overrides lists per-key RPM overrides.
	Overrides []APIKeyRateLimitEntry `yaml:"overrides,omitempty" json:"overrides,omitempty"`
}

// APIKeyRateLimitEntry maps a client API key to a custom RPM limit.

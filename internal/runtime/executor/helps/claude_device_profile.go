...
func ApplyClaudeLegacyDeviceHeaders(r *http.Request, incomingHeaders http.Header, cfg *config.Config, confirmedClaudeCode bool) {
	if r == nil {
		return
	}
	profile := defaultClaudeDeviceProfile(cfg)
	...
	if confirmedClaudeCode && !explicitlyDisabled {
		...
		r.Header.Set("X-Stainless-Os", incomingOrFallback("X-Stainless-Os", osFallback))
		r.Header.Set("X-Stainless-Arch", incomingOrFallback("X-Stainless-Arch", archFallback))
		r.Header.Set("User-Agent", softwareProfile.UserAgent)
		return
	}
	...
	r.Header.Set("X-Stainless-Runtime-Version", profile.RuntimeVersion)
	r.Header.Set("X-Stainless-Package-Version", profile.PackageVersion)
	r.Header.Set("X-Stainless-Os", osFallback)
	r.Header.Set("X-Stainless-Arch", archFallback)
	r.Header.Set("User-Agent", profile.UserAgent)
}

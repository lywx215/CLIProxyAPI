...
func TestAntigravityExecute_NoCreditsWithoutConductorFlag(t *testing.T) {
	...
	exec := NewAntigravityExecutor(&config.Config{
		QuotaExceeded: config.QuotaExceeded{AntigravityCredits: true, AntigravityCreditsForce: false},
	})
	auth := &cliproxyauth.Auth{
	...
	// Should NOT contain credits since conductor didn't request them and force is false
...
func TestAntigravityExecute_CreditsAlwaysInjectedWhenConfigEnabled(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	...
	// No conductor credits flag set in context — credits should still be injected
	// because antigravity-credits-force: true forces credit consumption on every request.
	_, err := exec.Execute(context.Background(), auth, cliproxyexecutor.Request{
		Model:   "claude-sonnet-4-6",
		Payload: []byte(`{"request":{"contents":[{"role":"user","parts":[{"text":"hi"}]}]}}`),
	}, cliproxyexecutor.Options{
	...
	// Credits SHOULD be injected because config has AntigravityCreditsForce: true (forced mode)
	if !strings.Contains(requestBodies[0], `"enabledCreditTypes"`) {
		t.Fatalf("request should contain enabledCreditTypes when config flag is enabled: %s", requestBodies[0])
	}
}

func TestAntigravityExecute_CreditsForceDoesNotInjectForNonClaude(t *testing.T) {
	resetAntigravityCreditsRetryState()
	t.Cleanup(resetAntigravityCreditsRetryState)
	...
	if strings.Contains(requestBodies[0], `"enabledCreditTypes"`) {
		t.Fatalf("non-Claude request should not contain enabledCreditTypes when config flag is enabled: %s", requestBodies[0])
	}
}

...
// buildRateLimitConfig converts per-API-key configuration into middleware state.
func buildRateLimitConfig(cfg *config.Config) middleware.RateLimitConfig {
	rateLimitCfg := middleware.RateLimitConfig{}
	if cfg == nil || !cfg.APIKeyRateLimit.Enabled {
		return rateLimitCfg
	}
	rateLimitCfg.DefaultRPM = cfg.APIKeyRateLimit.DefaultRPM
	if len(cfg.APIKeyRateLimit.Overrides) == 0 {
		return rateLimitCfg
	}
	rateLimitCfg.Overrides = make(map[string]int, len(cfg.APIKeyRateLimit.Overrides))
	for _, entry := range cfg.APIKeyRateLimit.Overrides {
		if key := strings.TrimSpace(entry.APIKey); key != "" && entry.RPM > 0 {
			rateLimitCfg.Overrides[key] = entry.RPM
		}
	}
	return rateLimitCfg
}

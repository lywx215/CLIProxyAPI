...
func TestAuditHomeCodexSearchBusyReturnsTrustedRetryAfter(t *testing.T) {
	...
	if !strings.Contains(rr.Body.String(), handlers.FixedErrorMessage(http.StatusTooManyRequests)) {
		t.Fatalf("body = %q, want sanitized rate-limit error", rr.Body.String())
	}
}
...
func TestHomeCodexAlphaSearchSanitizesUnauthorizedResponseWithoutRefresh(t *testing.T) {
	...
	if got := rr.Body.String(); got != string(handlers.BuildErrorResponseBody(http.StatusUnauthorized, "")) {
...
func TestBuildRateLimitConfigRespectsEnabled(t *testing.T) {
	cfg := &proxyconfig.Config{
		APIKeyRateLimit: proxyconfig.APIKeyRateLimit{
			DefaultRPM: 50,
			Overrides: []proxyconfig.APIKeyRateLimitEntry{
				{APIKey: "limited-key", RPM: 10},
			},
		},
	}

	disabled := buildRateLimitConfig(cfg)
	if disabled.DefaultRPM != 0 || len(disabled.Overrides) != 0 {
		t.Fatalf("disabled rate limit config = %+v, want no limits", disabled)
	}

	cfg.APIKeyRateLimit.Enabled = true
	enabled := buildRateLimitConfig(cfg)
	if enabled.DefaultRPM != 50 {
		t.Fatalf("enabled default RPM = %d, want 50", enabled.DefaultRPM)
	}
	if enabled.Overrides["limited-key"] != 10 {
		t.Fatalf("enabled override RPM = %d, want 10", enabled.Overrides["limited-key"])
	}
}
